...
$cek_kode=mysqli_query($conn,"select * from JobIM247 where JobTitle='$Title'");
...
echo "Your Job Title has already registered please use another Job Title";
   ...
   $sql="INSERT INTO JobIM247 (JobTitle,Date,NameOfCompany,Salary,Qualify,skill,Description) VALUES ( '$Title',' $Date','$Name','$Salary','$Qualify','$skill', '$Description')";
   if ($conn->query($sql) === TRUE) {
    echo "<script>alert('Submit Succesfully!');</script>";
    echo"<Script>window.location.replace('IM247Job.php');</script>";
  } else {
